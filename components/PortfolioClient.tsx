...
function CaseModal({ item, onClose }: { item: PortfolioItem; onClose: () => void }) {
  return (
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative max-w-2xl w-full matte-card-strong p-6 md:p-8 text-left">
        <button aria-label="Close" onClick={onClose} className="absolute top-3 right-3 p-2 text-white/70 hover:text-white">
          <X className="w-5 h-5" />
        </button>
        <div className="flex items-center gap-4 mb-4">
          <div className="relative w-20 h-12 rounded-md ring-1 ring-white/10 bg-black/70 overflow-hidden">
        ...
        <div className="mx-auto max-w-5xl grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
          ...
          {filtered.map((c) => (
            <article key={c.name} className="matte-card elevate-hover p-6 h-full relative overflow-hidden"
              ...
              {/* Minimal CTA */}
              <button onClick={() => setOpen(c.name)} className="mt-4 text-sm text-royal hover:text-accent-a transition-colors">
                View details →
              </button>
            </article>
          ))}
